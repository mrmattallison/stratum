...
-- lfo min/max menu params use this span so target sync (rate 1..7, clouds_pitch
-- −48..48, clouds_dens 0..100, …) does not clamp back into LFO.startup 0..1.
local LFO_PARAM_SPEC_LO = -200
local LFO_PARAM_SPEC_HI = 200
...
local function build_lfo_spec(lfo,i,bound)
  local def = bound == "min" and lfo.min or lfo.max
  def = util.clamp(def, LFO_PARAM_SPEC_LO, LFO_PARAM_SPEC_HI)
  params:add{
    type = 'control',
    id = 'lfo_'..bound..'_'..i,
    name = 'lfo '..bound,
...
function LFO:add_params(id,sep,group)
  if id ~= nil then
    -- norns: use lookup_param (params.lookup table is not always present)
    local _ok, _p = pcall(function() return params:lookup_param("lfo_"..id) end)
    if not _ok or _p == nil then
